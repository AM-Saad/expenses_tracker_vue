    ...
    <div class="wallet-content p-relative">
      ...
      <div class="p-relative " v-if="!loading">
        <div class="wallet-actions flex f-space-between p-3 card-actions">
          <div class="flex">
            <p class="m-medium">Card Name: <b>{{wallet.name}}</b></p>
            <p class="m-medium">Card Limit: <b>{{wallet.limit}}</b></p>
          </div>
        </div>
...
.wallet-transaction .in {
  background-color: rgb(159 255 159 / 67%)
}
.wallet-transaction .out {
  background-color:rgb(255 159 159 / 43%)
}
